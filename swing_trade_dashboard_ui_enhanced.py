...
for ticker in tickers:
    ...
    try:
    sma50_val = float(sma50) if pd.notna(sma50) else None
    sma200_val = float(sma200) if pd.notna(sma200) else None
    close_val = float(latest['Close']) if pd.notna(latest['Close']) else None

    if sma50_val is not None and sma200_val is not None and close_val is not None:
